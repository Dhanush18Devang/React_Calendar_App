...
const CalendarGrid = ({
  ...
  // ✅ Save Event + Highlight conflicts
  const saveEv = () => {
  ...
  const conflicts = events.filter((ev) => {
    ...
    if (editingEvent && ev === editingEvent) return false;

    const [evSh, evSm] = ev.startTime.split(":").map(Number);
    const [evEh, evEm] = ev.endTime.split(":").map(Number);
    const evStart = evSh * 60 + evSm;
    const evEnd = evEh * 60 + evEm;

    return !(endMinutes <= evStart || startMinutes >= evEnd);
  });

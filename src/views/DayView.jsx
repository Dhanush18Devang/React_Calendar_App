...
const DayView = ({ currentDate, events, onRightClick, onEventClick }) => {
  ...
  const todayEvents = events.filter((e) => dayjs(e.date).isSame(currentDate, "day"));
  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className="h-full overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-orange-400 scrollbar-track-gray-900 bg-black text-gray-200"
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-orange-400 pb-2 mb-2">
          <div className="text-xl font-bold text-orange-400">
            {currentDate.format("ddd").toUpperCase()}{" "}
            <span className="text-2xl text-white">{currentDate.format("D")}</span>
          </div>
        ...
        <div className="grid grid-cols-6 relative min-h-[1600px]">
          {/* Time column */}
          <div className="col-span-1 text-right pr-2 text-sm text-gray-400 border-r border-gray-700">
            {hours.map((hr) => (
              <div key={hr} className="h-16 border-t border-gray-800 pr-2">
          ...
          <div className="col-span-5 relative">
            {hours.map((hour) => (
              <div
                ...
                className="h-16 border-t border-gray-800 hover:bg-orange-950 cursor-pointer transition"
              />
            ))}
            ...
            {columns.map((column, colIndex) =>
              column.map((event, i) => {
                ...
                const duration = (eh + em / 60 - sh - sm / 60) * 4;
                ...
                return (
                  <div
                    ...
                    className={`absolute text-white text-opacity-95 p-2.5 rounded-lg shadow-md font-medium ${fontSize} cursor-pointer`}
                    style={{
                      top: `${top}rem`,
                      height: `${duration}rem`,
                      backgroundColor: event.color || hexToRgba("#f97316", 0.85), // default orange
